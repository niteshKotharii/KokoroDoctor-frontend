...
const EstablishmentTiming = ({ navigation }) => {
  ...
  const toggleFromPeriod = () => setFromPeriod(prev => (prev === 'AM' ? 'PM' : 'AM'));
  const toggleToPeriod = () => setToPeriod(prev => (prev === 'AM' ? 'PM' : 'AM'));
  ...
  <Text style={{ fontSize: 16, fontWeight: '500', color: '#000' }}>₹ 1999</Text>
</View>
...
>
  ...
  />
    ...
    <TouchableOpacity style={mobileStyles.continueButton}>
      <Text style={mobileStyles.continueText}
      onPress={() => navigation.navigate("DoctorCongrats")}>Continue →</Text>
    </TouchableOpacity>
